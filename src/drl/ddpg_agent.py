...
class Agent:
    ...
    def __init__(self, n_signals, window_length, n_assets, random_seed, lr_actor, lr_critic, buffer_size,
                 batch_size, tau, gamma, sigma, theta, fc1, fc2, pretrain, predict_type):
        ...
        self.actor_local = Actor(n_signals, window_length, n_assets,
                                 random_seed, fc1, fc2, predict_type, batch_size=1).to(device)
        self.actor_target = Actor(n_signals, window_length, n_assets,
                                  random_seed, fc1, fc2, predict_type, batch_size=1).to(device)
        if pretrain:
            self.actor_local.load_state_dict(torch.load('./weights/model/checkpoint_actor.pth'))
            self.actor_target.load_state_dict(torch.load('./weights/model/checkpoint_actor.pth'))
        self.actor_optimizer = optimum.Adam(self.actor_local.parameters(), lr=lr_actor)

        # Critic Network (w/ Target Network)
        self.critic_local = Critic(n_signals, window_length, n_assets,
                                   random_seed, fc1, fc2, predict_type, batch_size=256).to(device)
        self.critic_target = Critic(n_signals, window_length, n_assets,
                                    random_seed, fc1, fc2, predict_type, batch_size=256).to(device)
        if pretrain:
            self.critic_local.load_state_dict(torch.load('./weights/model/checkpoint_critic.pth'))
            self.critic_target.load_state_dict(torch.load('./weights/model/checkpoint_critic.pth'))
    ...
    def learn(self, experiences):
        ...
        # self.log.append([actor_loss, critic_loss])
